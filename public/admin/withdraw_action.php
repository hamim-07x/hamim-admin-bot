...
/** Placeholder tx hash for notification */
...
/**
 * Channel notify — bot style + autopay style combined:
 * image + premium emoji + User ID (no username) + Withdrawal #
 * + Amount + Address + Network + Status + Transaction + success line
 */
function notifyChannelRich(TelegramBot $bot, string $chat, array $ctx, string $photoUrl, array $extra): bool
{
    ...
    $idLink = emojiIdFor('ce_ref_2', '5271604874419647061');

    $statusLabel = strtoupper((string)$ctx['status']);
    if ($statusLabel === 'PAID') {
        $statusLabel = 'PAID SUCCESSFULLY';
    ...
    $parts = [
        ...
        ['e' => $idOk, 'f' => '✅'],
        ['t' => ' Status: '],
        ['b' => true, 't' => $statusLabel],
        ['t' => "\n\n"],
        ['e' => $idLink, 'f' => '🔗'],
        ['t' => " Transaction:\n"],
        ['code' => true, 't' => $ctx['txHash']],
        ['t' => "\n\n"],
        ['e' => $idOk, 'f' => '✅'],
        ['t' => ' '],
    ...
    $html .= ce('ce_payout_ok') . ' Status: <b>' . htmlspecialchars($statusLabel) . "</b>\n\n";
...
$statusLabel = ($newStatus === 'paid') ? 'PAID SUCCESSFULLY' : 'APPROVED';
...
$coreMsg .= ce('ce_receipt') . " Status: <b>{$statusLabel}</b>\n";
$coreMsg .= ce('ce_ref_2') . " Transaction:\n<code>" . htmlspecialchars($txHash) . "</code>";
