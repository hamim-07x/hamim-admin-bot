...
<?php if ($page === 'dashboard'): ?>
  ...
  <div class="card" style="margin-top:1rem"><p class="hint">Webhook: <b><?= ($settings['webhook_enabled'] ?? '0') === '1' ? 'ON' : 'OFF' ?></b> · Demo: <b><?= $demoOn ? 'ON' : 'OFF' ?></b> · Auto-pay: <b><?= ($settings['withdraw_mode'] ?? '') === 'auto' ? 'ON' : 'OFF' ?></b> · Currency: <b><?= htmlspecialchars($settings['currency_name'] ?? 'USDT') ?></b></p></div>
...
<?php elseif ($page === 'payment'): ?>
  ...
  <div class="card">
    <p class="hint"><b>2 · Auto-Pay Wallet (BEP-20)</b></p>
    <form method="post" action="/admin/save_settings.php">
  ...
  <div class="card">
    ...
    <form method="post" action="/admin/save_settings.php">
      ...
      <select name="auto_send_enabled" <?= $demoOn ? 'disabled' : '' ?>>
        ...
        <option value="1" <?= ($settings['auto_send_enabled'] ?? '0') === '1' ? 'selected' : '' ?>>ON — real BEP-20</option>
      </select>
  ...
  <div class="card">
    ...
    <form method="post" action="/admin/save_settings.php">
      ...
      <p class="hint" style="margin-top:.5rem">When ON: Auto-Approve & Auto-Send forced OFF. Users see processing → success. Channel gets demo TX hash. No tokens sent.</p>
