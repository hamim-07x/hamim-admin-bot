...
<?php if ($page === 'dashboard'): ?>
  ...
  <div class="stats-grid">
    ...
    <div class="stat-card">
      <div class="label">Joined 24h</div>
      <div class="value"><?= number_format($usersToday) ?></div>
      <div class="sub">Last 24 hours</div>
    </div>
    <div class="stat-card">
      <div class="label">Total Balance</div>
      <div class="value"><?= htmlspecialchars($s) . number_format($totalBalance, 2) ?></div>
      <div class="sub">All users · <?= htmlspecialchars($cName) ?></div>
    </div>
    <div class="stat-card">
      <div class="label">Pending WD</div>
      <div class="value"><?= number_format($wdPending) ?></div>
      <div class="sub">Awaiting approval</div>
    </div>
    <div class="stat-card">
      <div class="label">Paid / Approved</div>
      <div class="value"><?= number_format($wdPaid) ?></div>
      <div class="sub">Completed withdrawals</div>
    </div>
    <div class="stat-card">
      <div class="label">Rejected WD</div>
      <div class="value"><?= number_format($wdRejected) ?></div>
      <div class="sub">Refunded to users</div>
    </div>
    <div class="stat-card">
      <div class="label">Channels</div>
      <div class="value"><?= number_format($chCount) ?></div>
      <div class="sub">Join checklist</div>
    </div>
    <div class="stat-card">
      <div class="label">Referral Payouts</div>
      <div class="value"><?= number_format($refCount) ?></div>
      <div class="sub">Bonus credited</div>
    </div>
  </div>
  <div class="card" style="margin-top:1rem">
    <p class="hint">Webhook: <b><?= ($settings['webhook_enabled'] ?? '0') === '1' ? 'ON' : 'OFF' ?></b>
      ...
      · Network: <b><?= htmlspecialchars($settings['network'] ?? 'BEP20') ?></b>
  ...
  <div class="card">
    <form method="get" action="/admin/" class="search-bar">
      ...
      <div class="inline-form" style="margin-top:.35rem">
        ...
        <?php if ($q !== ''): ?>
          <a class="btn btn-ghost" style="text-decoration:none;padding:.6rem 1rem" href="/admin/?page=users">Clear</a>
        <?php endif; ?>
      </div>
    </form>
  </div>
  <div class="card"><div class="table-wrap"><table class="table">
      ...
      <tr>
        ...
        <td>
          ...
          <div class="user-meta"><?= htmlspecialchars($handle) ?> · ID <?= (int)$u['id'] ?></div>
        </td>
        ...
        <td>
          <form method="post" action="/admin/user_action.php" class="inline-form">
            ...
            <input name="amount" type="number" step="any" min="0" class="amt" placeholder="0" value="<?= htmlspecialchars((string)$u['balance']) ?>">
        ...
        <td>
          <form method="post" action="/admin/user_action.php" class="inline-form">
            <input type="hidden" name="user_id" value="<?= (int)$u['id'] ?>">
            <button name="action" value="<?= (int)$u['is_blocked'] ? 'unblock' : 'block' ?>" class="<?= (int)$u['is_blocked'] ? 'btn-ok' : 'btn-bad' ?>">
              <?= (int)$u['is_blocked'] ? 'Unblock' : 'Block' ?>
            </button>
          </form>
        </td>
      </tr>
      <?php endforeach; ?>
      <?php if (!$rows): ?>
      <tr><td colspan="7" style="color:var(--muted);text-align:center;padding:1.5rem">No users found</td></tr>
      <?php endif; ?>
  </table></div></div>
