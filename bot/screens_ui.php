...
function showWelcomeAgree(TelegramBot $bot, int $chatId, array $from): void
{
    ...
    $text  = ce('ce_welcome_1', 'W') . ' <b>' . htmlspecialchars($name) . '!</b> ' . $welcome . "\n\n";
    $text .= ce('ce_welcome_2', 'G') . " Complete tasks and earn rewards.\n";
    $text .= ce('ce_welcome_3', 'L') . " Invite friends to earn referral bonuses.\n";
    $text .= ce('ce_welcome_4', 'M') . " The More You Engage, The More You Earn.\n\n";
    $text .= ce('ce_welcome_5', '!') . " By continuing, you must agree to our Terms & Conditions.\n\n";
    $text .= ce('ce_welcome_6', 'v') . ' Click <b>I Agree & Continue</b> below.';
...
function showJoinScreen(TelegramBot $bot, int $chatId, int $userId, array $missing = []): void
{
    ...
    $welcome = getSetting('welcome_text', 'Welcome to Ton Grid');
    $text  = ce('ce_join_1', 'B') . ' <b>' . htmlspecialchars($welcome) . "</b>\n\n";
    $text .= ce('ce_join_2', 'C') . " <b>Join the channels below to continue.</b>\n";
    $text .= ce('ce_warn', '!') . " Only channels you have not joined are shown.\n";
    $text .= ce('ce_no', 'X') . " Without joining, the bot will not work.\n";
    $rows = []; $pair = [];
...
{
    if (!$missing) $missing = getMissingChannels($bot, $userId);
    $text  = ce('ce_join_no', 'X') . " <b>You must join these channels first</b>\n\n";
    $text .= ce('ce_warn', '!') . " To use the bot you must stay joined.\n";
    $text .= ce('ce_join_2', 'C') . " Only missing channels are listed:\n\n";
    foreach ($missing as $ch) $text .= ce('ce_join_1', '#') . ' @' . ltrim($ch['username'], '@') . "\n";
    $text .= "\n" . ce('ce_retry', 'R') . " Join them, then tap <b>Retry</b>.";
    $rows = []; $pair = [];
    foreach ($missing as $ch) {
        $link = trim((string)($ch['invite_link'] ?? ''));
        if ($link === '') $link = 'https://t.me/' . ltrim($ch['username'], '@');
...
{
    return TelegramBot::inlineKeyboard([[['text' => 'Back', 'callback_data' => 'go_menu']]]);
}

function showMainMenu(TelegramBot $bot, int $chatId, int $userId): void
{
    $text = ce('ce_menu_1', '*') . " <b>Main Menu</b>\n\n" . ce('ce_welcome_6', 'v') . ' Choose an option below:';
...
function showWallet(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    $text  = ce('ce_wallet_1', 'W') . " <b>{$c} Wallet</b>\n\n";
    $text .= "━━━━━━━━━━━━━━\n";
...
function showReferrals(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    $c = getSetting('currency_name', 'USDT');
    $s = getSetting('currency_symbol', '$');
    $text  = ce('ce_ref_1', 'R') . " <b>Refer & Earn Program</b>\n\n";
    $text .= ce('ce_ref_rocket', '>') . " Invite friends and earn!\n";
    $text .= ce('ce_ref_gift', 'G') . " You receive <b>{$s}{$bonus} {$c}</b> per valid join.\n\n";
    $text .= ce('ce_ref_2', 'L') . " <b>Your link:</b>\n<code>{$link}</code>\n\n";
...
function startWithdrawFlow(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    $s = getSetting('currency_symbol', '$');
    if ($bal < $min) {
        $text  = ce('ce_payout_1', 'P') . " <b>{$c} Payout</b>\n\n";
        $text .= ce('ce_payout_no', 'X') . " Insufficient balance.\n";
    ...
    $text .= ce('ce_balance', '$') . " Balance: <b>{$s}" . number_format($bal, 2) . "</b>\n";
    $text .= ce('ce_warn', '!') . " Minimum: <b>{$s}" . number_format($min, 2) . "</b>\n";
    $text .= ce('ce_network', 'N') . " Network: <b>BEP-20 (BSC)</b>\n\n";
    $text .= ce('ce_card', 'C') . ' Continue to enter wallet address?';
    $kb = TelegramBot::inlineKeyboard([
        [['text' => 'Payout', 'callback_data' => 'wd_continue'], ['text' => 'Cancel', 'callback_data' => 'wd_cancel']],
...
function handleWithdrawAddress(TelegramBot $bot, int $chatId, int $userId, string $address): void
{
    ...
    if (!preg_match('/^0x[a-fA-F0-9]{40}$/', $address)) {
        botSend($bot, $chatId, $userId, ce('ce_payout_no', 'X') . ' Invalid BSC address. Send 0x... or Cancel.');
        return;
    }
    $stmt = getDB()->prepare('SELECT balance FROM users WHERE id = ?');
    $stmt->execute([$userId]);
    ...
    $text  = ce('ce_payout_1', 'P') . " <b>Confirm Payout</b>\n\n";
    $text .= ce('ce_balance', '$') . " Amount: <b>{$s}" . number_format($bal, 2) . " {$c}</b> (MAX)\n";
    $text .= ce('ce_card', 'C') . " Address:\n<code>{$address}</code>\n\n";
    $text .= ce('ce_network', 'N') . " Network: <b>BEP-20 (BSC)</b>";
    $kb = TelegramBot::inlineKeyboard([[
...
function processWithdraw(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    if (!$address || !preg_match('/^0x[a-fA-F0-9]{40}$/', $address)) {
        botSend($bot, $chatId, $userId, ce('ce_payout_no', 'X') . ' Session expired.');
    ...
    try {
        ...
        if ($bal < $min) {
            $db->rollBack();
            botSend($bot, $chatId, $userId, ce('ce_payout_no', 'X') . ' Insufficient balance.');
            showMainMenu($bot, $chatId, $userId);
            return;
        }
        $amount = $bal;
        ...
        $db->commit();
        $c = getSetting('currency_name', 'USDT');
        $text  = ce('ce_payout_ok', 'Y') . " <b>Payout request submitted</b>\n\n";
        $text .= ce('ce_balance', '$') . " Amount: <b>{$amount} {$c}</b>\n";
        $text .= ce('ce_card', 'C') . " Address: <code>{$address}</code>\n";
        $text .= ce('ce_receipt', 'R') . ' Status: <b>Pending admin approval</b>';
        botSend($bot, $chatId, $userId, $text, 'img_payout', ['reply_markup' => backKeyboard()]);
    } catch (Throwable $e) {
        if ($db->inTransaction()) $db->rollBack();
        botSend($bot, $chatId, $userId, ce('ce_payout_no', 'X') . ' Error processing payout.');
        showMainMenu($bot, $chatId, $userId);
    }
}
...
{
    $text = ce('ce_earn_1', 'E') . " <b>EARN MORE</b>\n\n" . ce('ce_target', 'T') . " Tasks coming soon.\n" . ce('ce_fire', '*') . ' Stay tuned!';
