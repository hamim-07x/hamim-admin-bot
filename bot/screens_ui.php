...
/**
 * Inline UI only.
 * Back = previous step | Cancel = main menu (home)
 */
...
function mainMenuInline(): array
{
    ...
    return TelegramBot::inlineKeyboard([
        [inlineBtn(cleanBtnLabel($l['wallet']), 'nav_wallet', $idW), inlineBtn(cleanBtnLabel($l['referrals']), 'nav_referrals', $idR)],
        [inlineBtn(cleanBtnLabel($l['payout']), 'nav_payout', $idP), inlineBtn(cleanBtnLabel($l['earn']), 'nav_earn', $idE)],
    ]);
}
...
function backInline(): array
{
    ...
    return TelegramBot::inlineKeyboard([[inlineBtn('Back', 'go_menu', $idBack)]]);
}
...
function showWelcomeAgree(TelegramBot $bot, int $chatId, array $from): void
{
    ...
    $kb = TelegramBot::inlineKeyboard([[inlineBtn('I Agree & Continue', 'agree_continue', $idOk)]]);
...
{
    clearBotState($userId);
    setBotState($userId, 'withdraw_start');
    $stmt = getDB()->prepare('SELECT balance FROM users WHERE id = ?');
    $stmt->execute([$userId]);
    ...
    $text .= ce('ce_balance') . " Balance: <b>{$s}" . number_format($bal, 2) . "</b>\n";
    $text .= ce('ce_warn') . " Minimum: <b>{$s}" . number_format($min, 2) . "</b>\n";
    $text .= ce('ce_network') . " Network: <b>" . htmlspecialchars(paymentNetworkLabel()) . "</b>\n\n";
    $text .= ce('ce_card') . ' Continue to enter wallet address?';
    $kb = TelegramBot::inlineKeyboard([
        [inlineBtn('Payout', 'wd_continue', $idPayout), inlineBtn('Cancel', 'wd_cancel', $idCancel)],
...
function askWithdrawAddress(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    $c = getSetting('currency_name', 'USDT');
    $label = paymentNetworkLabel();
    $hint = payoutAddressHint();
    $ex = activePaymentNetwork() === 'ton' ? 'EQ...' : '0x...';
    $text  = ce('ce_btn_payout') . " <b>{$c} Payout</b>\n\n";
    $text .= ce('ce_card') . " Send your <b>{$label}</b> wallet address:\n\n";
    $text .= ce('ce_warn') . " Example: <code>{$ex}</code>";
    $idCancel = btnEmojiId('ce_btn_cancel', '5210952531676504517');
    $idBack = btnEmojiId('ce_btn_back', '5416041192905265756');
    $kb = TelegramBot::inlineKeyboard([[inlineBtn('Cancel', 'wd_cancel', $idCancel), inlineBtn('Back', 'wd_back_start', $idBack)]]);
    botSend($bot, $chatId, $userId, $text, 'img_payout', ['reply_markup' => $kb]);
}
...
function handleWithdrawAddress(TelegramBot $bot, int $chatId, int $userId, string $address): void
{
    ...
    if (!isValidPayoutAddress($address)) {
        $hint = payoutAddressHint();
        $text = ce('ce_payout_no') . ' Invalid address. Send a valid <b>' . htmlspecialchars(paymentNetworkLabel()) . '</b> address (' . htmlspecialchars($hint) . ') or Cancel.';
        $kb = TelegramBot::inlineKeyboard([[inlineBtn('Cancel', 'wd_cancel', $idCancel), inlineBtn('Back', 'wd_back_start', $idBack)]]);
        botSend($bot, $chatId, $userId, $text, 'img_payout', ['reply_markup' => $kb]);
        return;
    }
    ...
    $text .= ce('ce_network') . " Network: <b>" . htmlspecialchars(paymentNetworkLabel()) . "</b>";
    $kb = TelegramBot::inlineKeyboard([
        [inlineBtn('Confirm (MAX)', 'wd_confirm', $idOk), inlineBtn('Cancel', 'wd_cancel', $idCancel)],
...
function processWithdraw(TelegramBot $bot, int $chatId, int $userId): void
{
    ...
    if (!$address || !isValidPayoutAddress($address)) {
    ...
    try {
        ...
        $db->commit();
        if (tryAutoCompleteWithdrawal($bot, $chatId, $userId, $wdId, $amount, $address)) {
            return;
        }
        $c = getSetting('currency_name', 'USDT');
